...
begin
	...
	@constraint(natal, c_acucar,   200*x1 + 150*x2 + 150*x3 + 100*x4 <= 100000)
...
begin
	...
	# 2. Variáveis
	@variable(otimizabier, xp >= 0) # Pilsen
	@variable(otimizabier, xi >= 0) # IPA
	@variable(otimizabier, xb >= 0) # Bock

	# 3. Função Objetivo
	@objective(otimizabier, Max, 10*xp + 14*xi + 12*xb)

	# 4. Restrições (Note o uso de nomes para facilitar a análise posterior)
	@constraint(otimizabier, c_malte,    230*xp + 260*xi + 369*xb <= 500000)
	@constraint(otimizabier, c_lupulo,   1.7*xp + 9*xi   + 1*xb   <= 2500)
	@constraint(otimizabier, c_levedura, 0.6*xp + 0.9*xi + 0.1*xb <= 200)
	@constraint(otimizabier, c_agua,     1.6*xp + 1.5*xi + 1.5*xb <= 2000)
...
md"""
	...
	Imagine que o **Tio João** (irmão do pai do Pedro, que também tem um cervejaria cuja receita é a mesma da OtimziaBier) aparece na OtimizaBier e diz para o Pedro:
...
manifest_format = "2.0"
project_hash = "b6c4a3d9545f6b8aa0545447b36d7d9f8f9dd72a"
