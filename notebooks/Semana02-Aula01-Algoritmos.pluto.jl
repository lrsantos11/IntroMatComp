...
# ╔═╡ 5c1a7f8a-5197-4d6d-8603-8f73338ca42c
# (nova célula) -- Cabeçalho da seção
md"""
---
## Algoritmos Recursivos

Na última aula, calculamos somatórios com laços `for`. Há outra forma de pensar:

$$S(n) = \underbrace{1 + 2 + \cdots + (n-1)}_{S(n-1)} + n = S(n-1) + n, \quad S(0) = 0$$

O problema de tamanho $n$ se reduz ao de tamanho $n-1$. Essa é a ideia da **recursividade**.
...
function f(n)
    if <caso_base>
        ...
        return <expressao_com_f(n-1)>
    end
end
```
"""

# (nova célula) -- Somatório recursivo
md"""
### Exemplo 1: Somatório (conexão com a aula anterior)
"""
...
let
	n = 10
	@show somatorio_rec(n)
	@show n * (n + 1) ÷ 2   # fórmula fechada
end

# (nova célula) -- Fibonacci header
md"""
...
# (nova célula) -- Fibonacci recursivo
...
# (nova célula) -- primeiros 11 termos
[fibonacci_rec(i) for i ∈ 0:10]

# (nova célula) -- slider interativo
@bind N_fib Slider(0:30, default=10, show_value=true)

# (nova célula) -- resultado interativo
"F($N_fib) = $(fibonacci_rec(N_fib))"

# (nova célula) -- comparação de tempo
md"""
### Problema de eficiência

`fibonacci_rec(n)` faz aproximadamente $2^n$ chamadas. Vejamos na prática:
"""
...
function fibonacci_iter(n)
	...
	a, b = 0, 1
	for i ∈ 2:n
		a, b = b, a + b   # atualiza ambos simultaneamente
	end
	return b
end

# (nova célula) -- comparação cronometrada
let
	n = 35
	...
	Recursivo:  $(round(t_rec  * 1e3, digits=2)) ms
	Iterativo:  $(round(t_iter * 1e6, digits=3)) μs
	Razão:      $(round(t_rec / t_iter)) ×
	"""
end

# (nova célula) -- MDC header
md"""
### Exemplo 3: MDC -- Algoritmo de Euclides

Um dos algoritmos mais antigos da matemática (~300 a.C.):
...
function mdc(a, b)
	if b == 0
		...
		return mdc(b, mod(a, b))   # passo recursivo
	end
end

# (nova célula) -- rastreamento
let
	a, b = 48, 18
	passos = Tuple{Int,Int}[]
	function mdc_verbose(a, b)
		push!(passos, (a, b))
		b == 0 ? a : mdc_verbose(b, mod(a, b))
	end
	resultado = mdc_verbose(a, b)
	println("Passos: $passos")
	println("mdc($a, $b) = $resultado")
end

# (nova célula) -- sliders para MDC
md"**Brinque com os valores:**"
...
| | Recursão | Iteração |
|---|---|---|
| **Legibilidade** | Espelha a definição matemática | Pode ser mais verbosa |
| **Memória** | Usa a pilha de chamadas | Usa variáveis locais |
| **Risco** | *Stack overflow* para $n$ grande | Segura para $n$ grande |
| **Quando usar** | Estrutura inerentemente recursiva | Laços simples, desempenho |

> **Regra prática:** se a definição matemática é recursiva, comece pela versão recursiva. Se o desempenho for crítico, converta para iterativa.
"""
