...
# ╔═╡ f03bf7c7-f00b-401d-850a-874c070608f9
using StatsPlots
...
md"""
### Gráficos em Julia - Parte 2
"""

# ╔═╡ d2bbb2af-b9b2-44ce-a1e4-a0201d2b396f
md"""
### Retomando a aula anterior

Na aula passada vimos:
- `plot` para traçar curvas ou ligar pontos;
- `scatter` para mostrar pontos isolados;
- gráficos de funções;
- sobreposição de gráficos com `plot!`.

Hoje vamos focar em:
- gráficos para dados;
- comparação entre visualizações;
- leitura de arquivos e gráficos com dados reais;
- salvamento de figuras.
"""

# ╔═╡ e10c24de-43d4-4e5d-a1a6-451a531eb569
let
    x = 1:10
    y = rand(10)

    plt1 = plot(x, y, label="plot", title="Ligando os pontos", lw=2)
    plt2 = scatter(x, y, label="scatter", title="Mostrando os pontos", ms=4)

    plot(plt1, plt2, layout=(1,2), size=(900,300))
end

# ╔═╡ 0720d615-2f86-4ef4-abdf-e56b2dd3c84c
md"""
### Desenhando um Quadrado
"""

# ╔═╡ 99561366-a516-4c9a-943e-0081a00b5b88
let
	CoordXQuadrado = [0, 1, 1, 0, 0]
	CoordYQuadrado = [0, 0, 1, 1, 0]
	plot(CoordXQuadrado,CoordYQuadrado, leg = false, aspect_ratio = :equal, 
		 fill = true, #Preencher a cor
		 colour = :yellow
		)
...
# ╔═╡ 43d22d82-dd4a-4625-a879-1bb0f9c8a12b
md"""
#### Curva da Borboleta
"""

# ╔═╡ 1de1cff7-f6fa-44da-b73d-63fb16f1663d
let
	# Gráfico da borboleta
	fx(t) = sin(t)*(exp(cos(t)) - 2cos(4t) - sin(t/12)^5)
    fy(t) = cos(t)*(exp(cos(t)) - 2cos(4t) - sin(t/12)^5)
	tt = range(0, 12π, 1000) # Valores espaçados entre 0 e 12π
	plot(fx.(tt), fy.(tt),
		linewidth = 2,
		legend = false,
		color = :purple,
		title = "Curva da Borboleta")
...
# ╔═╡ a7c8b15c-471d-474d-84a8-7f2c9302e902
...
"""
data = 0.1:.1:5 
# data = range(0.1,5, length= 10)

# ╔═╡ 45fbece4-10c4-4243-a086-40da3e90d845
begin 
	f(x) = log(x)
	plt1 = plot(f, 0.1, 5, title = "Gráfico de log(x)", xlabel = "x", ylabel = "log(x)", legend = false)
	scatter!(data,f.(data))
end
...
begin 
	g(x) = exp(x)
	plt2 = plot(g, 0.1, 5, title = "Gráfico de exp(x)", xlabel = "x", ylabel = "exp(x)", legend = false)
	scatter!(data,g.(data))
end
...
begin
    ...
    plt_metro = scatter(
        ...
        xlabel = "Longitude",
        ylabel = "Latitude",
        title = "Estações de metrô de São Paulo",
        aspect_ratio = :equal,
        legend = false
    )

    for (x, y) in zip(lon_metro, lat_metro)
        annotate!(x, y, text("M", 7, :white, :center, :middle))
    end
...
savefig(plt_metro, "../assets/metro sp.pdf")
...
manifest_format = "2.0"
project_hash = "a36182fe4f7146a71eeb3be57e915f7466d4c624"
