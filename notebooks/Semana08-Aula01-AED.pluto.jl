...
# ╔═╡ f35d739c-75c4-46cd-abf9-170bbdf5c56e
using MLDatasets
...
Note que a sintaxe `df_iris.Species` (acesso por propriedade) faz exatamente a mesma coisa que a sintaxe `df_iris[:, :Species]` que aprendemos na Semana 07. A primeira é um atalho prático muito usado em Análise Exploratória, enquanto a segunda é a forma rigorosa de extrair um vetor de uma matriz no Julia!
"""
...
md"""
E se existirem dados faltantes? Na vida real, a Análise Exploratória serve justamente para identificar esses buracos. Quando os encontramos, temos duas opções principais com o `DataFrames.jl`:
1. **Descartar as linhas incompletas:** Usando a função `dropmissing(df)`
2. **Preencher os buracos:** Usando a função `coalesce` para trocar o `missing` por um valor padrão (como 0 ou a média).
"""
...
md"""
### Visualizando a Distribuição Estatística
Tudo o que vimos numericamente com o `summarystats` e o agrupamento pode ser perfeitamente visualizado usando gráficos como o `boxplot`. Essa é a essência da EDA: cruzar variáveis categóricas (espécies) com distribuições contínuas (tamanho da pétala).
"""
...
# ╔═╡ 02085551-6ec6-4dbe-a77a-e904954fb7ee
...
md"""
## Exercício da Aula 01 — Análise Exploratória (MTCars)

> Carregue o clássico dataset de carros usando `df_carros = dataset("datasets", "mtcars")`.
> Em seguida, realize o ciclo completo de EDA abordado hoje:
> 1. Use `summarystats` para entender a escala de Cavalos de Potência (coluna `:HP`) e de Milhas por Galão (coluna `:MPG`).
> 2. Verifique se existe algum dado faltante na tabela.
> 3. Use o `groupby` e o `combine` para descobrir a Média de Milhas por Galão (MPG) agrupada pelo Número de Cilindros (coluna `:Cyl`).
> 4. Faça um Gráfico de Dispersão (`scatter` usando a macro `@df`) cruzando `:HP` no eixo X e `:MPG` no eixo Y, agrupando as cores (group) pelo número de cilindros (`:Cyl`).

**Observação:** Suba o Notebook Pluto com suas resoluções no Moodle conforme orientado na página da disciplina.
"""
...
manifest_format = "2.0"
project_hash = "7994b88a92ad646cc8e8953ce8383845d06333bf"
...
git-tree-sha1 = "653ea50df05f4df3913bc3c51f895127ae391b03"
